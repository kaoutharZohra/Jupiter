...
. Deploy EnhacementFingerprint.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy EnhacementFingerprint.ctf

1.1 Prerequisites to Deployment
Before you can deploy EnhacementFingerprint.ctf you need a server instance to host it. 
The server instance must be configured to have access to MATLAB runtime 8.5.
...
1.2 EnhacementFingerprint.ctf Deployment
To deploy EnhacementFingerprint.ctf copy it into the server instance's `auto_deploy` 
folder. The server instance will automatically deploy it and make it available to 
interested clients.
